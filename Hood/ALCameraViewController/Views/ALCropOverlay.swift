...
internal class ALCropOverlay: UIView {
    ...
    override func layoutSubviews() {
        ...
        for i in 0..<horizontalLines.count {
            ...
            let hSpacing = (bounds.height/3 * CGFloat(i + 1)) + (lineThickness * CGFloat(i))
            let vSpacing = (bounds.width/3 * CGFloat(i + 1)) + (lineThickness * CGFloat(i))
            hLine.frame = CGRectMake(0, hSpacing, size.width, lineThickness)
            vLine.frame = CGRectMake(vSpacing, 0, lineThickness, size.height)
        }
