...
public class ALCameraView: UIView {
    ...
    public func capturePhoto(completion: ALCameraShotCompletion) {
        dispatch_async(cameraQueue) {
            ALCameraShot().takePhoto(self.imageOutput, videoOrientation: self.preview.connection.videoOrientation) { image in
                
                var correctedImage = image
                print(image.size)
                if self.currentPosition == AVCaptureDevicePosition.Front {
                    correctedImage = UIImage(CGImage: image.CGImage!, scale: image.scale, orientation: UIImageOrientation.Right)
